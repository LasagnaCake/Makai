...
int main() {
	...
	if(withinBounds(Point(0), Box(0, 1)))			DEBUGLN("    Point-to-Box OK!");
	if(withinBounds(Point(.5), Circle(0, 1)))		DEBUGLN("    Point-to-Circle OK!");
	...
	if(!withinBounds(Point(-1), Box(1, 1)))			DEBUGLN("    Point-to-Box OK!");
