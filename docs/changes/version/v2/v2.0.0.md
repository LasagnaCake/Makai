...
More information on [flow.purplecheshire.net](https://flow.purplecheshire.net).

### Differences
- In FLOW, separators are not needed, and are discarded.
- Line comments (`//`) & block comments (`/* ... */`).
- Strings & keys can be several types of strings, or unquoted (but composed only of alphanumeric characters or underscore).
...
- `<format>`: String format encoding specification. Must be a power of 2, up to 64.
...
- **Linux:** Debian packages are available.

## OpenCL support

Via the `Makai::MP::OpenCL` namespace.

## Image Processing Facilities

Via the `Makai::Image` namespace.

## Video Processing Facilities

Via the `Makai::Video` namespace.
...
## Removed old archive executables (`arcgen`, `arcpack`, `arcunpack`) and consolidated in a single app: `packer`

Binaries are shipped with every release.
...
- Complete overhaul on `Thread` class
- Removed `Promise` class → now use `Thread::Promise` instead
- Removed `Task` class → `Thread` class can handle it now
- Reworked `Color` namespace:
- - Moved to `ctl/ex/color/` → now header-only
- - Renamed (almost) all functions
- - Added support for Fixed-Width Integer RGB & RGBA colors (2, 3, 5, 8, 16, 24, 32, 48, 64)
