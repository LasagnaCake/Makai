...
public:
	...
	/// @param other Other `Result` to copy from.
	constexpr Result(SelfType&& other)							{operator=(move(other));	}
	...
	constexpr SelfType& operator=(SelfType&& other) {
		switch (other.state) {
			case ResultState::RS_OK:	return operator=(move(other.result.value));
			case ResultState::RS_ERROR: return operator=(move(other.result.error));
			default: destruct(); break;
		}
		return *this;
	}
	...
	/// @brief Order comparison operator (value).
	/// @param value Value to compare.
	/// @return Order between `Result`s.
	constexpr OrderType operator<=>(DataType const& value) const {
		if (state == ResultState::RS_UNDEFINED) return Order::LESS;
		return ok() ? result.value <=> value : Order::LESS;
	}
	/// @brief Order comparison operator (error).
	/// @param value Value to compare.
	/// @return Order between `Result`s.
	/// @note Requires error type to not be implicitly convertible to result type.
	constexpr OrderType operator<=>(ErrorType const& error) const requires (!IMPLICIT) {
		if (state == ResultState::RS_UNDEFINED) return Order::LESS;
		return ok() ? Order::GREATER : result.error <=> error;
	}
	/// @brief Order comparison operator (`Result`).
	/// @param value Value to compare.
	/// @return Order between `Result`s.
	constexpr OrderType operator<=>(SelfType const& other) const {
		switch (state) {
			case ResultState::RS_ERROR:		return other <=> result.error;
			case ResultState::RS_VALUE:		return other <=> result.value;
			default: return state <=> other.state;
		}
	}
	...
	/// @brief Returns whether there is no identifiable value.
	/// @return Whether there is no identifiable value.
	constexpr bool inLimbo() const		{return state == ResultState::RS_UNDEFINED;	}
	...
	/// @brief T Data type.
	template <class T>
	using ReturnType = Meta::If<Type::Constructible<T, nulltype>, T, Nullable<T>>;
	...
	constexpr DataType const& operator*() const {
		if (!ok())
	...
	/// @return Stored value.
	/// @throw Error::NonexistentValue if value is not set.
	constexpr ref<DataType const> operator->() const {
		if (!ok())
			throw Error::NonexistentValue("Value is not set!", CTL_CPP_PRETTY_SOURCE);
		return &result.value;
	}
	...
	/// @return Stored value.
	/// @throw Error::NonexistentValue if value is not set.
	constexpr ref<DataType> operator->() {
		if (!ok())
			throw Error::NonexistentValue("Value is not set!", CTL_CPP_PRETTY_SOURCE);
		return &result.value;
	}
