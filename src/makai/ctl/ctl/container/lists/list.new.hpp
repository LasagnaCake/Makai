#ifndef CTL_CONTAINER_LISTS_LIST_H_NEW
#define CTL_CONTAINER_LISTS_LIST_H_NEW
...
public:
	...
	constexpr List(As<ConstantType[S]> const& values) {
		invoke(S);
		excopy(contents.data(), 0, values, S);
		count = S;
	}
	...
	constexpr List(SelfType const& other) {
		...
		excopy(contents.data(), 0, other.contents.data(), other.count);
		count = other.count;
	}
	...
	requires Type::Different<T2, DataType> {
		if (end <= begin) return;
		invoke(end - begin + 1);
		excopy(contents.data(), 0, begin, end - begin);
		count = end - begin;
	}
...
//		requires !Type::Constructible<T, ConstIteratorType, ConstIteratorType>;
	...
	constexpr SelfType& insert(DataType const& value, IndexType index) {
		...
		excopy(contents.data(), index + 1, contents.data() + index, count - index);
	...
	constexpr SelfType& insert(SelfType const& other, IndexType index) {
		...
		wrapBounds(index, count);
		expand(other.count);
		excopy(contents.data(), index+other.count, contents.data() + index, count - index);
		excopy(contents.data(), index, other.contents.data(), other.count);
		count += other.count;
		return *this;
	}
	...
	constexpr SelfType& resize(SizeType const newSize) {
		...
		if (contents.size())	changeSize(newSize);
	...
	constexpr SelfType& appendBack(ConstIteratorType const& begin, ConstIteratorType const& end) {
		expand(end - begin);
		excopy(contents.data(), count, begin, end - begin);
		count += (end - begin);
		return *this;
	}
	...
	constexpr SelfType& appendBack(As<DataType[S]> const& values) {
		expand(S);
		excopy(contents.data(), count, values, S);
		count += S;
		return *this;
	}
...
private:
	...
	constexpr void invoke(SizeType const size) {
		CTL_DEVMODE_FN_DECL;
	...
	constexpr SelfType& squashRange(SizeType start, SizeType const amount) {
		...
		if (count > 1 && start < count-amount)
			for (; start < (count-amount); ++start)
				MX::reconstruct(contents.data() + start, contents.data()[start+amount]);
		MX::objclear(contents.data()+start, amount);
		return *this;
	}

	constexpr static void excopy(ref<DataType> const dst, SizeType const start, ref<DataType const> const src, SizeType const count) {
		CTL_DEVMODE_FN_DECL;
		if (Type::Standard<DataType> && inRunTime())
			MX::memmove<DataType>(dst, src + start, count);
		else MX::objcopy<DataType>(dst, src + start, count);
	}

	constexpr StorageType createCopy(SizeType const newBufferSize) {
		CTL_DEVMODE_FN_DECL;
		StorageType buffer{newBufferSize};
		if (!count) return buffer;
		auto const toCopy = newBufferSize < count ? newBufferSize : count;
		excopy(buffer.data(), 0, contents.data(), toCopy);
		return buffer;
	}

	constexpr StorageType moveToNewBuffer(SizeType const newBufferSize) {
		CTL_DEVMODE_FN_DECL;
		StorageType buffer{newBufferSize};
		if (!count) return buffer;
		auto const toCopy = newBufferSize < count ? newBufferSize : count;
	...
	constexpr void changeSize(SizeType const newSize) {
		CTL_DEVMODE_FN_DECL;
		if (!newSize)		dump();
		else if (!count)	contents.resize(newSize);
		else 				contents = moveToNewBuffer(newSize);
	}
