...
void Context::debugExternalFunction(bool const isExiting) {
	if (isExiting) {
		MAKAILIB_DEBUGLN_FULL("<<<<<<<<<<<<<<<<<<<<<< Destroying external function...");
	} else {
		MAKAILIB_DEBUGLN_FULL(">>>>>>>>>>>>>>>>>>>>>> Creating external function...");
	}
}
...
bool Context::openLibrary(Makai::String const& path) {
	...
	MAKAILIB_DEBUGLN_FULL("Fetching library...");
...
void Context::loadLibraries() {
	MAKAILIB_DEBUGLN_FULL("Loading libraries...");
	for (auto& lib: toBeLoaded) {
		MAKAILIB_DEBUGLN_FULL("<", lib->name(), ">");
		lib->load(*this);
		MAKAILIB_DEBUGLN_FULL("</", lib->name(), ">");
	}
	toBeLoaded.clear();
	MAKAILIB_DEBUGLN_FULL("Done loading libraries!");
}
...
bool Context::addNativeCall(usize const hash, usize const argc, ExternalInvocation const& invoker) {
	MAKAILIB_DEBUGLN_FULL("Adding method [", hash, "]");
	if (hasNativeCall(hash)) {
		MAKAILIB_DEBUGLN_FULL("WARN: [", hash, "] duplicate found");
		return false;
	}
	MAKAILIB_DEBUGLN_FULL("OK: [", hash, "] has no duplicates");
...
Context::MethodResult Context::callNative(usize const hash, List<Object::Storage> const& args) {
	MAKAILIB_DEBUG_BLOCK_FULL {
		...
		for (auto& m: externalMethods)
			MAKAILIB_DEBUGLN_FULL("  > ", m.key);
	}
	MAKAILIB_DEBUG_BLOCK_FULL {
		MAKAILIB_DEBUGLN_FULL("Method exists? ", hasNativeCall(hash));
		MAKAILIB_DEBUGLN_FULL("Registered? ", externalMethods.contains(hash));
		if (externalMethods.contains(hash)) {
			MAKAILIB_DEBUGLN_FULL("Created? ", externalMethods[hash].exists());
...
bool Context::Library::Impl::open(Makai::String const& path, Context& context) {
	...
	MAKAILIB_DEBUGLN_FULL("Opening library...");
	dll.open(path);
	MAKAILIB_DEBUGLN_FULL("Getting entrypoint...");
	auto const fn = dll.function<owner<ALibrary>()>("AV2_Extern_getLibrary");
	lib = fn();
	if (!lib) return false;
	MAKAILIB_DEBUGLN_FULL("<library>");
	MAKAILIB_DEBUGLN_FULL("<name>", lib->name(), "</name>");
	MAKAILIB_DEBUGLN_FULL("<version>", lib->version().serialize().toFLOWString(), "</version>");
	MAKAILIB_DEBUGLN_FULL("</library>");
	lib->open();
	return true;
}
