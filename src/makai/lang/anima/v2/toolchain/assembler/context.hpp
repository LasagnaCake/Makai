...
namespace Makai::Anima::V2::Toolchain::Assembler {
	struct BaseContext {
		...
		struct Axiom: Tokenizer::Token {
			...
			constexpr static Axiom invalid() {
				return {{Type::LTS_TT_INVALID, {}}, true, "<INVALID>"};
			}
		};
		...
		protected:
			virtual void display(String const& str)	{MAKAILIB_DEBUG_FULL(str);	}
		};
		...
		Axiom peek(usize const ahead = 0) {
			if (tokens.size() < (ahead + 2))
				return Axiom::invalid();
			return tokens[-(ahead+2)];
		}

		BaseContext& next() {
			if (empty()) return *this;
			tokens.popBack();
			if (!empty()) MAKAILIB_DEBUGLN_FULL("Token: ", token().text);
			return *this;
		}
