...
namespace Makai::Anima::V2::Toolchain::Compiler::Breve {
	...
	struct EnumResolver: AResolver {
		EnumResolver(): AResolver() {}
		virtual ~EnumResolver() {}
		Node::Instance resolve(Parser& parser, Node::Instance const& lhs, BaseContext::Axiom const& token) override;
	};
	...
	struct DropExpressionResolver: AResolver {
		DropExpressionResolver(): AResolver() {}
		virtual ~DropExpressionResolver() {}
		Node::Instance resolve(Parser& parser, Node::Instance const& lhs, BaseContext::Axiom const& token) override;
	};
	...
	struct LambdaResolver: AResolver {
		LambdaResolver(): AResolver(Parser::Precedence::AV2_TAPP_DECL, false) {}
		virtual ~LambdaResolver() {}
		Node::Instance resolve(Parser& parser, Node::Instance const& lhs, BaseContext::Axiom const& token) override;
	};
	...
	struct ReificationResolver: AResolver {
		ReificationResolver(): AResolver() {}
		virtual ~ReificationResolver() {}
		Node::Instance resolve(Parser& parser, Node::Instance const& lhs, BaseContext::Axiom const& token) override;
	};

	struct UnionResolver: AResolver {
		UnionResolver(): AResolver() {}
