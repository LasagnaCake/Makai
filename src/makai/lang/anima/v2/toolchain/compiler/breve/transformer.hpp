...
namespace Makai::Anima::V2::Toolchain::Compiler::Breve::Transformer {
	...
	struct ComparisonExpression: ATransformer {
		Result transform(Context& context, Node::Instance const& node) override;
	};
	...
	struct Import: ATransformer {
		...
		static Makai::Function<File(UTF8String const&)> importer;
	};
	...
	struct TupleTypeDecl: ATransformer {
		Result transform(Context& context, Node::Instance const& node) override;
	};

	struct NullableTypeDecl: ATransformer {
		Result transform(Context& context, Node::Instance const& node) override;
	};
	...
	struct ShortMatch: ATransformer {
		Result transform(Context& context, Node::Instance const& node) override;
	};
