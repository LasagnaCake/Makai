...
float noise(vec2 texelUV) {
    return simplenoise(texelUV + fragCoord3D.xy + fragNormal.xy);
}
...
void main(void) {
    ...
    if (blendTexture.enabled) {
        ...
        switch (blendTexture.equation & 0x30u) {
            // Default equations
            case 0x00:
            result = equate((src), (dst), blendTexture.equation);
            break;
            // Destination-inverse equations
            case 0x10:
            result = equate((src), (1 - dst), blendTexture.equation);
            break;
            // Source-inverse equations
            case 0x20:
            result = equate((1 - src), (dst), blendTexture.equation);
            break;
            // Full-inverse equations
            case 0x30:
            result = equate((1 - src), (1 - dst), blendTexture.equation);
            break;
            // Fallback
            default:
            break;
    ...
    if (debugView > 0) {
        switch (debugView) {
            case 1:
            FragColor = vec4(fragNormal / 2 + 0.5, 1);
            break;
            default:
            break;
        }
